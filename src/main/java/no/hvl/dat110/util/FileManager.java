...
public class FileManager {
	...
	public int distributeReplicastoPeers() throws RemoteException {
		...
		int index = rnd.nextInt(numReplicas); // randomly pick primary
		int counter = 0;
		...
		for (int i = 0; i < numReplicas; i++) {
			...
			succ.addKey(replicaID);
			
			boolean isPrimary = (i == index);
			succ.saveFileContent(filename, replicaID, bytesOfFile, isPrimary);
			counter++;
		}
	...
	public Set<Message> requestActiveNodesForFile(String filename) throws RemoteException {
		...
		for (BigInteger replica : replicafiles) {
			...
			Message msg = succ.getFilesMetadata().get(replica);
			if (msg != null) {
	...
	public NodeInterface findPrimaryOfItem() {
		try {
			...
			logger.error("Could not find primary node: " + e.getMessage());
		}
		return null;
	}
