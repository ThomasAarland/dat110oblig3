...
public class ChordProtocols {
	...
	public void fixFingerTable() {
		try {
			...
			fingerTable.clear();  // clear existing table
	
			BigInteger addressSize = Hash.addressSize(); // 2^128
			int m = Hash.bitSize(); // 128
	
			BigInteger nodeID = chordnode.getNodeID();
	
			for (int i = 0; i < m; i++) {
				BigInteger offset = new BigInteger("2").pow(i);
				BigInteger id = nodeID.add(offset).mod(addressSize);
	
				NodeInterface succ = chordnode.findSuccessor(id);
